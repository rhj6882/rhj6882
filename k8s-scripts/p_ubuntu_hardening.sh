...
if [ ! -f /etc/ssh/sshd_config ]; then
  echo "⚠️ sshd_config 파일이 없어 openssh-server 설치합니다."
...
maxretry = 5
EOF
  echo "✅ Fail2Ban SSH 보호 설정 추가됨"
else
  echo "⚡ Fail2Ban SSH 보호 설정 이미 존재함"
fi
...
if ! grep -q "pam_pwquality.so" /etc/pam.d/common-password; then
  ...
  echo "✅ pam_pwquality 설정 추가됨"
else
  echo "⚡ pam_pwquality 설정 이미 존재함"
fi
...
if command -v firewall-cmd &> /dev/null; then
  ...
  echo "⚡ firewalld가 설치되어 있지 않아 스킵합니다."
fi
